...
export default function RoundRobin(players, startingRound = 1, ordered = false) {
  let matches = [];
  let playerArray = [];
  ...
  for (let r = startingRound; r < startingRound + playerArray.length - 1; r += 1) {
    const round = [];
    for (let i = 0; i < playerArray.length / 2; i += 1) {
      round.push({
        round: r,
        match: i + 1,
        player1: null,
